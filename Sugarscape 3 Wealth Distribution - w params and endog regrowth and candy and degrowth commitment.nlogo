...
turtles-own [
  ...
  candy           ;; the amount of candy this turtle has
  ...
  work-time       ;; the quantity of ticks spend on working by each turtle
  work-hist       ;; history of worked ticks
  ;;neighbors-nature;; psugar/max-psugar in neighborhood
  utility         ;; utility hehe
  degrowth        ;;
  ;;eaten-candy
]
...
to turtle-setup ;; turtle procedure
  ...
  set work-time random-in-range 0 3
  set candy random-in-range minimum-sugar-endowment maximum-sugar-endowment
  set metabolism min-met + (random-float max-met)
  set vision random-in-range min-vision max-vision
  set max-age random-in-range min-max-age max-max-age
  ;;set neighbors-nature 0
  set age 0
  set work-hist [0 0 0]
  set utility 0
  set degrowth 0
  if random-float 100 < degrowth-committed [
    set degrowth 1
    set color green
  ]
...
to go
  ...
  ask turtles [
    turtle-move
    ifelse degrowth = 0 [turtle-work] [
      ifelse candy < 3 * metabolism [turtle-work] [
        ifelse ((psugar - required-sugar) * (1 + sugar-rate) >= psugar ) [turtle-work] [
          set work-hist lput 0 (but-first work-hist)
        ]
      ]
    ]
    turtle-eat-metabolism
    ;;turtle-nature
    turtle-utility
    set age (age + 1)
    if candy <= 0 or age > max-age [
...
to turtle-work ;; turtle procedure
  ;; work for 3 ticks to achieve 1 candy from 3 units of sugar
  if (work-time < 3) and (psugar >= required-sugar) [
    set psugar psugar - required-sugar
  ...
  set work-hist lput 1 (but-first work-hist)
;;  set eaten-candy 0

end

to turtle-eat-metabolism ;; turtle procedure
  ;; metabolize some sugar, and eat all the sugar on the current patch
  set candy (candy - metabolism)

end

;;to turtle-eat ;; turtle procedure
  ;; eat candy just for pleasure
;;  set eaten-candy (random-in-range 1 20) / 10
;;  set candy (candy - eaten-candy)
;;  set work-hist lput 0 (but-first work-hist)

;; end

;;to turtle-nature
;;  set neighbors-nature (sum [psugar] of neighbors) / (max (list 1 (sum [max-psugar] of neighbors)))
;;end

to turtle-utility
  set utility (alpha * candy) - (beta * sum(work-hist))
end
...
to patch-growback ;; patch procedure
  ...
  set psugar min (list max-psugar (psugar * (1 + sugar-rate)))
end

to update-lorenz-and-gini
  let num-people count turtles
  let sorted-wealths sort [candy] of turtles
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot sum [candy] of turtles"
